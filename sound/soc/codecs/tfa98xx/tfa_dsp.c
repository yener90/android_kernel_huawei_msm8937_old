...
/* Define the speaker impedance. */
#define TFA_IMDEPENDENCE_VALUE	7
...
/* Calibration fail retry optimization. */
int  CalibrationFailedRetry = 3;
//static int  CalibrationSucessful = 0;


int  AlreadyResetMTPEX = 0;
...
int tfa98xx_powerdown(struct tfa98xx *tfa98xx, int powerdown)
{
	...
	/* print log to confirm powerdown settings is ok or not. */
...
static int tfa9890_specific(struct tfa98xx *tfa98xx)
{
    ...
    /* added current sense ktemp value to 7BE1 as indicated in datasheet */
...
static int tfa98xx_startup(struct tfa98xx *tfa98xx)
{
	...
	*/
    ...
    if (tries == AREFS_TRIES) {
		...
		pr_debug("statusreg = 0x%04x, tries=%d\n", statusreg, tries);
	}
...
/* Default range: (R0(8Ohm) ¡À 20%) * 1000;
 * The range should be set according to the defination of speaker vendor.
 */
#define imped_min (6400)
#define imped_max (9600)

int tfa98xx_check_impedance(int ime)
{
    return (ime >= imped_min && ime <= imped_max);
}

/*
 * reset MtpEx for cleaning the result(flags) of calibration.
 */
 int tfa98xx_reset_MTPEX(struct tfa98xx *tfa98xx)
{
	int status;
	int tries, ret;
	...
	status = snd_soc_read(codec, TFA98XX_MTP);
		pr_debug("TFA98XX_MTP is 0x%04x\n", status);
		ret = 0;
	}
...
int tfa98xx_coldboot(struct tfa98xx *tfa98xx, int state)
{
	...
	while (state == !tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tries-- == 0) {
			pr_warn("coldboot (ACS) did not %s\n",
				  state ? "set" : "clear");
			return -EINVAL;
		}
	}
...
int tfa98xx_dsp_power_up(struct tfa98xx *tfa98xx)
{
	...
	if (tries == 0) {
		...
		pr_debug("DSP subsystem start tries: %d\n", (CFSTABLE_TRIES - tries));
	}
...
static int tfa98xx_set_mute(struct tfa98xx *tfa98xx, enum Tfa98xx_Mute mute)
{
	...
	/* print system control register for check. */
	pr_debug("sysctrl_value = 0x%04x\n", sysctrl_value);
	...
	*has_drc = 0;
	...
	if (tfa98xx->has_drc) {
		*has_drc = tfa98xx->has_drc;
		...
		if ((tfa98xx->rev == REV_TFA9887 && tfa98xx_is87(tfa98xx)) || (tfa98xx->rev == REV_TFA9890)) {
...
int tfa98xx_dsp_get_calibration_impedance(struct tfa98xx *tfa98xx, u32 *re25)
{
 ...
 int tfa98xx_aec_output(struct tfa98xx *tfa98xx, int enable)
{
	struct snd_soc_codec *codec = tfa98xx->codec;
	u16 value;
	int ret = 0;
...
static int tfaRunSetCalibrateOnce(struct tfa98xx *tfa98xx)
{
	...
	if ((mtp & TFA98XX_MTP_MTPOTC) == 0) {
		...
		for (tries = 0; tries < TFA98XX_WAITRESULT_NTRIES_WAIT; tries++) {
			msleep_interruptible(10);
			/* Check if MTP is still busy */
			status = snd_soc_read(codec, TFA98XX_STATUSREG);
			if (!(status & TFA98XX_STATUSREG_MTPB_MSK))
...
int tfaRunSpeakerBoost(struct tfa98xx *tfa98xx, int force)
{
	...
	pr_debug("force: %d, dsp_init: %d.\n", force, tfa98xx->dsp_init);
	...
	if (force || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (!done) {
			...
			ret = tfa98xx_dsp_get_calibration_impedance(tfa98xx, &re25);
			if(ret == 0){
				pr_info("calibration impedance = %d mOhm\n", re25);
				if(tfa98xx_check_impedance(re25)){
					pr_info("Calibration done and successful!\n");
					CalibrationFailedRetry = 0;
					tfa98xx->dsp_init = TFA98XX_DSP_INIT_DONE;
				}else if(CalibrationFailedRetry > 0){
					pr_warn("Calibration done but impedance is invalid!\n");
					done = 0;
					tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;
					if(AlreadyResetMTPEX == 0){
						tfa98xx_reset_MTPEX(tfa98xx);	//if speaker calibration failed, reset MTPEX for only one time
						AlreadyResetMTPEX = 1;
					}
					CalibrationFailedRetry--;
				}
				else {
					pr_err("check impedance invalid and init failed.\n");
					done = 0;
					tfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
				}
			}else{
				pr_err("get impedance failed, calibration not done!\n");
				done = 0;
				if(CalibrationFailedRetry > 0){
					CalibrationFailedRetry--;
					tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;
					...
					pr_err("retry calibration timeout and init failed.\n");
					tfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
				}
			}
		}
		...
		if (ret || (tfa98xx->dsp_init == TFA98XX_DSP_INIT_FAIL)){
			pr_err("Not calibrated yet! ret: %d.\n", ret);
			done = 0;
		}
		else
			done = 1;
...
int tfa98xx_dsp_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (forcecoldboot || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tfaRunSpeakerBoost(tfa98xx, forcecoldboot)){
			pr_err("cold start up error! return.\n");
			if((tfa98xx->dsp_init == TFA98XX_DSP_INIT_FAIL) && (CalibrationFailedRetry > 0))
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;
			return -EINVAL;
		}
	} else {
...
int tfa98xx_dsp_stop(struct tfa98xx *tfa98xx)
{
	...
	/* dummy read in order to generate i2c clocks */
	...
	{
		status = snd_soc_read(codec, TFA98XX_STATUSREG);
		pr_err("mute device error: %d, status reg = 0x%04x\n", ret, status);
		/* if waiting timeout, don't return it, unless i2c error. */
		if(ret != -ETIMEDOUT)
			return ret;
	}

...
static unsigned char read_flash_data(unsigned int address) {
	...
	buf = (unsigned char)(inwrite(0x041012));
	return buf;
}
...
int read_product_id(void)
{
	...
	if (NULL == i2c.product_id) {
		...
		ret = outwrite(0x04100B, 0x03, 1);
		ret = outwrite(0x041009, 0x00, 2);
		for (i = 0xE000; i <= 0xE006; i++) {
			i2c.product_id[i - 0xE000] = read_flash_data(i);
			tp_log_info("ilitek flash_buf[0x%X] = 0x%X\n", i, i2c.product_id[i - 0xE000]);
		}

...
int et6326_breath_leds_time(struct aw2013_dev_data *aw2013){
	//set breath led flash time from blink
	int err=0;
	if(aw2013->leds[LED_RED].blinking)
	{
 	switch (aw2013->leds[LED_RED].blinking)
      {
		case 1:
			aw2013_i2c_write(0x1, 0x14);//rase time 2.5s
			aw2013_i2c_write(0x2, 0x33);//dry flash period
			aw2013_i2c_write(0x5, 0x11);//reset internal counter
			break;
		case 2:
			aw2013_i2c_write(0x1, 0x18);//rase time 3.5s
			aw2013_i2c_write(0x2, 0x01);//dry flash period
			aw2013_i2c_write(0x5, 0x11);//reset internal counter
			break;
		default:
			break;
  ...
  {
	switch (aw2013->leds[LED_GREEN].blinking)
     {
		case 1:
			aw2013_i2c_write(0x1, 0x14);//rase time 2.5s
			aw2013_i2c_write(0x2, 0x33);//dry flash period
			...
			break;
		case 2:
			aw2013_i2c_write(0x1, 0x1b);//rase time 3.5s
			aw2013_i2c_write(0x2, 0x25);//dry flash period
			aw2013_i2c_write(0x5, 0x11);//reset internal counter
			break;
		default:
			break;
...
int aw2013_set_color_singlecolor(struct aw2013_dev_data *aw2013)
{
	...
	unsigned char blink_flag = 0,blink_flagred=0,blink_flaggreen=0,blink_flagblue=0;
	if(aw2013->leds[LED_RED].brightness ||aw2013->leds[LED_GREEN].brightness || aw2013->leds[LED_BLUE].brightness)
	{	
		...
		if (aw2013->leds[LED_RED].brightness && aw2013->leds[LED_GREEN].brightness)
			{
				red_on = 1;
				green_on = 0;
			}
	...
	if(aw2013->leds[LED_RED].blinking ||aw2013->leds[LED_GREEN].blinking || aw2013->leds[LED_BLUE].blinking)
	{
        ...
        if (aw2013->leds[LED_BLUE].blinking)
			...
			aw2013_i2c_write(0x4,0x0);
			aw2013_i2c_write(0xa,0x3);
			aw2013_i2c_write(0xa,0);
	if(red_on || green_on || blue_on)
	{
		if(0 == aw2013_has_inited){
			aw2013_i2c_write(0x0,0x18);
			mdelay(1);
			aw2013_has_inited = 1;  
		}
			led_sutdown=0;	
            ...
            if(red_on)
			{
			aw2013_i2c_write(0x6, 0x20);
			aw2013_i2c_write(0x8, 0x20);
			}
			if(green_on)aw2013_i2c_write(0x7, 0x20);
            break;
            case 0x34:  //yellow
            if(red_on)
			{
    ...
    if(blink_flag){
			et6326_breath_leds_time(aw2013);
			aw2013_i2c_write(0x4, (blink_flagred)<<5|(blink_flaggreen)<<3|(blink_flagred)<<1);
    		
	}else {
			aw2013_i2c_write(0x4, red_on<<4|green_on<<2|red_on);	// LED level, 
	} 
	}
	else{	
			aw2013_i2c_write(0x4,0);//shut down led module
			aw2013_i2c_write(0xa,0x3);
			blink_flag=0;
            led_sutdown=1;
	}
...
static int ET6326_led_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s_aw2013->regulator = devm_regulator_get(&client->dev,"rgb_led");
	if (IS_ERR(s_aw2013->regulator)) {
		dev_err(&client->dev, "regulator get failed\n");
		err = PTR_ERR(s_aw2013->regulator);
		s_aw2013->regulator = NULL;
		return err;
	}
	if(0 != aw2013_power_up(s_aw2013, client, true))
	{
		dev_err(&client->dev, "regulator power up failed\n");
		err = -ENODEV;
		goto exit2;
	}
#if 0
...
static int ET6326_led_remove(struct i2c_client *client)
{
	int count = 0;
...
static const struct i2c_device_id ET6326_led_id[] = {
	{ET6326_I2C_NAME, 0 },
	{ }
};
void ET6326_led_shutdown(struct i2c_client *client)
{
	aw2013_i2c_write(0x4,0x0);
	aw2013_i2c_write(0xa,0x3);//reset part of register
	aw2013_i2c_write(0xa,0);
	printk("successfully shutdown.");
};
static int led_suspend(struct device *dev)
{
	...
	if (led_sutdown) 
	{
		if(0 != aw2013_power_up(s_aw2013, client, false))
		{
			return 0;
		}
	}
...
static struct of_device_id ET6326_led_match_table[] = {
	{ .compatible = "awinc,et6326", },
	{ },
};

static struct i2c_driver ET6326_led_driver = {
	.probe		= ET6326_led_probe,
	.remove		= ET6326_led_remove,
	.shutdown	= ET6326_led_shutdown,
	.id_table	= ET6326_led_id,
	.driver = {
		.name	= ET6326_I2C_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = ET6326_led_match_table,
		.pm		= &led_pm_ops,
	},
};

static int __init ET6326_led_init(void)
{
	pr_info("ET6326 led driver: initialize.");
	return i2c_add_driver(&ET6326_led_driver);
}

static void __exit ET6326_led_exit(void)
{
	pr_info("ET6326 led driver: release.");
	i2c_del_driver(&ET6326_led_driver);
}

subsys_initcall(ET6326_led_init);
module_exit(ET6326_led_exit);

MODULE_AUTHOR("ming he <heming@wingtech.com>");
MODULE_DESCRIPTION("ET6326 driver");
MODULE_LICENSE("GPL");

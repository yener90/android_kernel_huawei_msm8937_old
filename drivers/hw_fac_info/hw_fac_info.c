...
static char hardwareversion[64];
static int __init early_parse_hardwareversion(char *cmdline)
{
	if (cmdline) {
		strlcpy(hardwareversion, cmdline, sizeof(hardwareversion));
	}

	return 0;
}
early_param("androidboot.hardwareversion", early_parse_hardwareversion);
...
static int  fac_info_probe(struct platform_device *pdev)
{
	...
	if(strlen(hardwareversion) != 0) {
		ret = set_hardware_ver_to_appinfo_cmdline(hardwareversion);
	}
	else	 {

...
int set_hardware_ver_to_appinfo_cmdline(char * hard_ver)
{
	if(strlen(hard_ver) != 0)
	{
		if (-1 == app_info_set("huawei_hardware_version", hard_ver)) {
			pr_err("%s: set hardware version to app info fail!\n", __func__);
			return DT_PROP_ERROR;
		}
	}
	...
	{
		return set_hardware_ver_to_appinfo();
	}

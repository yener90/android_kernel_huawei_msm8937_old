...
int qpnp_pon_set_restart_reason(enum pon_restart_reason reason)
{
	...
	pr_err("PMIC pon_reset_reason: 0x88F mask=%x,setting %x\n",PON_MASK(7, 2),(reason << 2));
	if (rc)
		dev_err(&pon->spmi->dev,
...
{
	extern unsigned int zwy_debug_detected_long_key;
	extern struct task_struct *zwy_debug_kpdpwr_tk;
#ifdef CONFIG_MSM_DLOAD_MODE
	/* clear dload mode to reduce false triggering dump*/
	//clear_dload_mode();
#endif
	pr_info("long press power key have detected!\n");
	zwy_debug_detected_long_key = 0x55aacc33;
	mb();
	wake_up_process(zwy_debug_kpdpwr_tk);
...
unsigned int zwy_debug_detected_long_key = 0;
struct task_struct *zwy_debug_kpdpwr_tk = NULL;
static int debug_detect_long_kpdpwr(void *_data)
{
	struct file *fp;
	char time_buf[128];
	char file_name[128];
	...
	while (!kthread_should_stop()) {
		if (zwy_debug_detected_long_key != 0x55aacc33) {
			set_task_state(current, TASK_UNINTERRUPTIBLE);
			schedule();
		}
		zwy_debug_detected_long_key = 0;
		do_gettimeofday(&tv);
		get_current_time(time_buf, sizeof(time_buf));
		snprintf(file_name, sizeof(file_name), "/data/hwzd_logs/input-log/long_press_%lu.txt", (unsigned long)tv.tv_sec);
		fp = filp_open(file_name, O_SYNC | O_RDWR | O_CREAT, 00644);
		if (!IS_ERR(fp)) {
			vfs_write(fp, time_buf, strlen(time_buf), &pos);
			filp_close(fp, NULL);
		}
		else {
			pr_err("zwy, can not creat %s\n", file_name);
		}
		fp = NULL;
	}
	return 0;
...
{
	zwy_debug_kpdpwr_tk = kthread_run(debug_detect_long_kpdpwr, NULL, "kzwykpdpwr");
	if (!IS_ERR(zwy_debug_kpdpwr_tk)) {
		struct sched_param param;
		param.sched_priority = 1;
		sched_setscheduler(zwy_debug_kpdpwr_tk, SCHED_FIFO, &param);
		pr_info("zwy, long_press_kpdpwr_task running..\n");
	}

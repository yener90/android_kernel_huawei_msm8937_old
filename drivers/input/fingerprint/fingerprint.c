...
static long fingerprint_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case FP_IOC_CMD_SEND_SENSORID:
            ...
            break;
       case FP_IOC_CMD_WAKE_LOCK :
	   	//wake_lock_timeout(&fingerprint->ttw_wl, msecs_to_jiffies(FPC_TTW_HOLD_TIME*10));
		wake_lock_timeout(&fingerprint->waitup,msecs_to_jiffies(3000));
		 fpc_log_info("time WAKE_LOCK\n");
	   	break;
		
	case FP_IOC_CMD_WAKE_UNLOCK :
	   	wake_unlock(&fingerprint->waitup);
		 //fpc_log_info("time WAKE_UNLOCK\n");
	   	break;
        default:
...
static int fingerprint_get_module_info(struct fp_data* fingerprint)
{
    ...
    //udelay(1000);
    msleep(5);
    pu_value = gpio_get_value(fingerprint->moduleID_gpio);
    ...
    pd_value = gpio_get_value(fingerprint->moduleID_gpio);
...
/*     error = fingerprint_power_init(fingerprint);
   ...
   error = fingerprint_get_module_info(fingerprint);
    ...
    wake_lock_init(&fingerprint->waitup, WAKE_LOCK_SUSPEND, "fpc_waitup");
...
#endif
    if( fingerprint->module_vendor_info == MODULEID_LOW)
    	{
    	  app_info_set("fingerprint", "Syna-109A");
    	}
   else if( fingerprint->module_vendor_info == MODULEID_FLOATING)
	{
	  app_info_set("fingerprint", "Goodix-gf3288");
	}
    fpc_log_info("fingerprint probe is successful!\n");
    // printk("xxxx fingerprint probe is successful!\n");
    return error;

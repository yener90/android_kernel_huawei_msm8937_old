...
#define FTS_CSV_FILE_PATH "/sdcard/"
...
static int fts_i2c_read(unsigned char *writebuf,
...
static int fts_i2c_write( unsigned char *writebuf, int writelen)
{
    int ret;
...
static int fts_test_entry_show(char *ini_file_name, char *bufdest, ssize_t* pinumread)
{
	...
	for(i=0;i<3;i++){
		if(true == start_test_tp()){
			FTS_TEST_DBG("wdb: fts tp test pass");
			test_result = 0;
		}
		else{
			FTS_TEST_DBG("wdb: fts tp test failure");
			test_result = -1;
			}
	...
	*/
	...
	fts_test_save_test_data("fts_tcl_testdata.csv", testdata, iTestDataLen);
...
{
	return fts_test_entry_show( ini_file_name, bufdest, pinumread);
}

...
//Add,@wangdongbo.wt_ttp_open_short_test
#define CTP_PROC_OPEN_SHORT_TEST	1
...
#define FULL_RAW_CAP_TEST_LOWER 300        
#define FULL_RAW_CAP_TEST_UPPER 70000
#define NOISE_TEST_TEST_FRAMES 55 //50
#define NOISE_TEST_LOWER  28
#define NOISE_TEST_UPPER  28
#define EE_SHORT_TEST_LOWER  93  //96
#define EE_SHORT_TEST_UPPER  105  //100
#define AMP_OPEN_TEST_INT_DUR_ONE 145      // integration duration
#define AMP_OPEN_TEST_INT_DUR_TWO 10       // integration duration
#define AMP_OPEN_TEST_PHASE_ONE_LOWER 50  //500  // ( unit = femtofarad (fF) )
#define AMP_OPEN_TEST_PHASE_ONE_UPPER 10000 //3000 // ( unit = femtofarad (fF) )
#define AMP_OPEN_TEST_PHASE_TWO_LOWER 20  //70   // ( unit = ratio )
#define AMP_OPEN_TEST_PHASE_TWO_UPPER 500 // 130  // ( unit = ratio )
...
static int td43xx_ee_short_normalize_data(signed short * image)
{
	...
	for (i = 0; i < rx_num; i++) {
		...
		//pr_info("right_median[%d] = %d\n", i, right_median[i]);	
		//pr_info("left_median[%d] = %d\n", i, left_median[i]);	
	}
...
static ssize_t synaptics_ee_short_test(void)
{
	int retval;
	int i, j, offset;
	int tx_num = f54->tx_assigned;
	...
	unsigned char *td43xx_ee_short_data = NULL;
	unsigned int td43xx_rt95_report_size = tx_num * rx_num * 2;	
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	bool result = 1;

	//step 1: ee_short_test_store
	...
	retval = test_sysfs_read_report_td43xx(NULL, NULL, "95", 2, 
					false, false);
	if(retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	for (i = 0, offset = 0; i < tx_num * rx_num; i++) {
		...
		td43xx_rt95_part_one[i] = (td43xx_rt95_part_one[i] > EE_SHORT_TEST_UPPER) ? 1 :0;
		offset += 2;
	}
	...
	td43xx_ee_short_normalize_data(td43xx_rt95_part_two);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (td43xx_ee_short_data[i * tx_num + j] != 0) {
				result &= 0;
				dev_err(f54->rmi4_data->pdev->dev.parent, "%s: E-E Short failed on (%-2d, %-2d)\n",					
					__func__, i, j);				
				//dev_err(f54->rmi4_data->pdev->dev.parent,
				//	"0x%02x\n",
				//	td43xx_ee_short_data[i * tx_num + j]);
			}
		}
	}
...
exit:
	...
	td43xx_rt95_part_two = NULL;
	td43xx_rt95_part_one = NULL;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned int noise_report_size = tx_num * rx_num * sizeof(short);
	unsigned long setting;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	int rx_num = f54->rx_assigned;
	bool result = 1;
	const unsigned int noise_upper_limit = 28;
	...
	for (i = 0; i < tx_num; i++){
		for (j = 0; j < rx_num; j++){
			if (td43xx_noise_delta[i*rx_num + j] > noise_upper_limit) {
				result &= 0;
				//pr_notice("failed on %d %d (%d)\n", i, j, td43xx_noise_delta[i*rx_num + j] );
			}
		}
	}
...
/*
static ssize_t synaptics_full_raw_test(void)
{
	int retval;
	int i, j;
	ssize_t ret = 0;
	...
	unsigned short *report_data_16;
       ...
       dev_err(rmi4_data->pdev->dev.parent,"synaptics: synaptics_full_raw_test 11 \n");
	...
	retval = test_sysfs_read_report_td43xx(NULL, NULL, "92", 2, 
				false, false);
	if(retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read report. exit\n",
	...
	for (i = 0; i < tx_num; i++) {		
		for (j = 0; j < (rx_num - 1); j++) {			
			if ((*report_data_16 > FULL_RAW_CAP_TEST_UPPER) || (*report_data_16 < FULL_RAW_CAP_TEST_LOWER))	{				
				result &= 0;				
				dev_err(f54->rmi4_data->pdev->dev.parent, "%s: Full Raw Cap failed on (%-2d, %-2d), data : %-4d\n",
					__func__, i, j, *report_data_16);	
			}			
			report_data_16++;		
		}		
		if ((*report_data_16 > FULL_RAW_CAP_TEST_UPPER) || (*report_data_16 < FULL_RAW_CAP_TEST_LOWER))	{
...
exit:
	...
	td43xx_full_raw_data = NULL;
       ...
       dev_err(rmi4_data->pdev->dev.parent,"synaptics: synaptics_full_raw_test 22 \n");
...
static int td43xx_amp_open_data_testing(signed short * p_image, 
		...
		signed int ph2_lower_limit, signed int ph2_upper_limit)
{
	...
	if (f54->swap_sensor_side) {
		...
		for (i = 0; i < rx_num; i++) {
			for (j = 0; j < right_size; j++) {
				p_right_column_buf[i * left_size + j] = p_data_16[j * rx_num + i];  
			}
		}
		...
		for (i = 0; i < tx_num; i++) {
			for (j = 0; j < rx_num; j++) {
				...
				if ((p_image[i * rx_num + j] < ph1_lower_limit) || (p_image[i * rx_num + j] > ph1_upper_limit)) {
					...
					pr_info("%s : phase 1 failed at row:%d, col:%d\n", __func__, i, j);
				}
				/* phase 2 comparsion */
				/* the ratio should be within the ph2 lower and upper linit */
					...
					pr_info("%s : phase 2 failed at row:%d, col:%d (ratio = %d)\n", __func__, i, j, temp);
				}
				else {
...
static int synaptics_amp_open_test(void)
{
	int retval = 0;
	int i, j, k;
	int tx_num = f54->tx_assigned;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			{
				dev_err(f54->rmi4_data->pdev->dev.parent, "%s: AMP open test failed on (%-2d, %-2d), data : 0x%02x\n",
					__func__, i, j, td43xx_amp_open_data[i*tx_num + j]);			
				result &= 0;
			}
		}
	}
...
exit_amp_open:
	...
	dev_err(rmi4_data->pdev->dev.parent," synaptics open_test  result=%d \n",result);
	//return snprintf(buf, PAGE_SIZE, "%s\n", (result == 1) ? "PASS" : "FAIL");
	return result;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			dev_dbg(f54->rmi4_data->pdev->dev.parent,
				"0x%02x\n",
...
//Add,@wangdongbo.wt_20160714_open_short_test
#if CTP_PROC_OPEN_SHORT_TEST
#include <linux/proc_fs.h>
#define CTP_PARENT_PROC_NAME  "touchscreen"
#define CTP_OPEN_PROC_NAME        "ctp_openshort_test"

static ssize_t synaptics_open_short_test(void)
{
    ssize_t res=0;
    ssize_t i=0;
     ssize_t result= 0;
    ...
    //step 1: amp_open_test    
    res = synaptics_amp_open_test();
    if(res<0)
    {
    	dev_err(rmi4_data->pdev->dev.parent,"open_test error \n");
    }
    else
    {
    	i++;
    	dev_err(rmi4_data->pdev->dev.parent,"open_test ok = %ld \n",res);
    }

    //step 2: ee_short_test
    res = synaptics_ee_short_test();
    if(res<0)
    {
    	dev_err(rmi4_data->pdev->dev.parent,"short_test error \n");
    }
    else
    {
    	i++;
    	dev_err(rmi4_data->pdev->dev.parent,"short_test ok = %ld \n",res);
    }
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
    ...
    if(*ppos)
    {
        dev_err(rmi4_data->pdev->dev.parent,"tp test again return\n");
        return 0;
    }
    *ppos += count;
   ...
   dev_err(rmi4_data->pdev->dev.parent,"TP Test Begin.\n");

   result =  synaptics_open_short_test();
 
   dev_err(rmi4_data->pdev->dev.parent,"TP Test result =%d \n",result);
...
static void create_ctp_proc(void)
{
    ...
    {
        dev_err(rmi4_data->pdev->dev.parent,"synaptics: create parent_proc fail\n");
        return;
    }
    ...
    {
        dev_err(rmi4_data->pdev->dev.parent,"synaptics: create open_proc fail\n");
    }
}
#endif

...
#include <linux/input/synaptics_dsx_v2_6_8.h>
...
//printk switch
#define SYNAPTICS_PRINTK_DEBUG		1
#define SYNAPTICS_GESTURE_SUPPROT	1

#ifdef SYNAPTICS_GESTURE_SUPPROT
#include <linux/input.h>

#define KEY_GESTURE_DOUBLECLICK			KEY_F1
#define KEY_GESTURE_C						KEY_F8
#define KEY_GESTURE_E 						KEY_F9
...
#define GESTURE_MASK					0x2781
#define GESTURE_BIT(i)					(1<< i)
#define GESTURE_MASK_DOBLUECLICK		GESTURE_BIT(0)
#define GESTURE_MASK_C					GESTURE_BIT(13)
#define GESTURE_MASK_E					GESTURE_BIT(13)
#define GESTURE_MASK_M				GESTURE_BIT(13)
#define GESTURE_MASK_W				GESTURE_BIT(13)

#define SYNAP_LIMIT_GESTURE_SIZE
#ifdef SYNAP_LIMIT_GESTURE_SIZE
/* user defined */
#define HEIGHT_LIMIT 200
#define WIDTH_LIMIT   200
/* gesture track record */
#define START_POINT_X 0x400
#define START_POINT_Y 0x402
#define END_POINT_X   0x404
#define END_POINT_Y   0x406
/* gesture size record */
#define UPPER_POINT_X 0x408
#define UPPER_POINT_Y 0x40A
#define LEFT_POINT_X  0x40C
#define LEFT_POINT_Y  0x40E
#define LOWER_POINT_X 0x410
#define LOWER_POINT_Y 0x412
#define RIGHT_POINT_X 0x414
#define RIGHT_POINT_Y 0x416
#endif


#define PANELID_MAX_ITEM_LONGTH	128
static unsigned short coordinate_x[6] = {0};
static unsigned short coordinate_y[6] = {0};
static u16 synap_easy_wakeup_gesture = 0;
static struct synaptics_rmi4_data *synap_rmi4_data;

static bool check_single_gesture_onoff(u16 mask)
{
if((synap_easy_wakeup_gesture & mask ) != 0)
	return true;
else
	return false;
}
...
#ifdef SYNAPTICS_GESTURE_SUPPROT
bool synap_gesture_onoff(void)
{
if( (synap_easy_wakeup_gesture & GESTURE_MASK ) != 0)
	return true;
else
	return false;
}
...
EXPORT_SYMBOL(synap_gesture_onoff);
#endif
...
static int easy_wakeup_gesture_report_position(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	unsigned char f51_custom_data[24] = {0};
	unsigned int f51_custom_baseaddr = 0x400;
	int i = 0;

	for(i = 0; i<24; i++)
	{
		retval = synaptics_rmi4_reg_read(rmi4_data,
				f51_custom_baseaddr+i,
				f51_custom_data+i,
				1);
		if (retval < 0)
		{
			printk("f51_data_base_addr write error!retval = %d \n",retval);
			return retval;
		}
	}
	mutex_lock(&rmi4_data->sysfs_mutex);
	for(i = 0;i < 6;i ++)
	{
			coordinate_x[i] = ((f51_custom_data[4 * i + 1] << 8) | (f51_custom_data[4 * i + 0]));
			coordinate_y[i] = ((f51_custom_data[4 * i + 3] << 8) | (f51_custom_data[4 * i + 2]));
	}
	mutex_unlock(&rmi4_data->sysfs_mutex);

	return 0;
}
static void synaptics_rmi4_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	switch(gesture_id)
	{
			case GESTURE_DOUBLECLICK:
					if(check_single_gesture_onoff(GESTURE_MASK_DOBLUECLICK))
					{
						input_report_key(input_dev, KEY_F1, 1);
						input_sync(input_dev);
						input_report_key(input_dev, KEY_F1, 0);
						input_sync(input_dev);
						#ifdef CONFIG_LOG_JANK
					...
					break;
			case GESTURE_C:
					if(check_single_gesture_onoff(GESTURE_MASK_C))
					{
						input_report_key(input_dev, KEY_F8, 1);
						input_sync(input_dev);
						input_report_key(input_dev, KEY_F8, 0);
						input_sync(input_dev);
					}
					break;
			case GESTURE_E:
					if(check_single_gesture_onoff(GESTURE_MASK_E))
					{
						input_report_key(input_dev, KEY_F9, 1);
						input_sync(input_dev);
						input_report_key(input_dev, KEY_F9, 0);
						input_sync(input_dev);
					}
					break;
			case GESTURE_M:
					if(check_single_gesture_onoff(GESTURE_MASK_M))
					{
						input_report_key(input_dev, KEY_F10,1);
						input_sync(input_dev);
						input_report_key(input_dev, KEY_F10, 0);
						input_sync(input_dev);
					}
					break;
			case GESTURE_W:
					if(check_single_gesture_onoff(GESTURE_MASK_W))
					{
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	int temp;
#ifdef SYNAP_LIMIT_GESTURE_SIZE
unsigned short upper_y=0,lower_y=0;
unsigned short left_x=0,right_x=0;
int vertical_y=0,horizontal_x=0;
#endif
...
#ifdef SYNAP_LIMIT_GESTURE_SIZE
	if (rmi4_data->gesture_detection[0] != 0x03)//NOT DOUBLECLICK
	{
		dev_err(rmi4_data->pdev->dev.parent,
			"wdb: start read gesture coordidate \n");
		...
		horizontal_x = right_x -left_x ;
		vertical_y = lower_y - upper_y;

		dev_err(rmi4_data->pdev->dev.parent,
			"horizontal_x = %d, vertical_y = %d \n",
			horizontal_x,vertical_y);

		if(horizontal_x < 0)
		{
			horizontal_x = left_x - right_x;
		}

		if(vertical_y < 0)
		{
			vertical_y = upper_y - lower_y;
		}
		...
		if(horizontal_x < WIDTH_LIMIT || vertical_y < HEIGHT_LIMIT){
			dev_err(rmi4_data->pdev->dev.parent,
						"%s: Gesture size is not enough!\n",
						__func__);
			return 0;
		}
	}
...
#if SYNAPTICS_PRINTK_DEBUG
printk("rmi4_data->suspend = %d ,  rmi4_data->enable_wakeup_gesture=%d \n",rmi4_data->suspend,rmi4_data->enable_wakeup_gesture);
for(loop=0;loop<5;loop++)
	printk("gesture_detection[%d]=%d \n",loop,rmi4_data->gesture_detection[loop]);
#endif

		if (rmi4_data->gesture_detection[0]==0x03)
		{
		#if SYNAPTICS_PRINTK_DEBUG
			printk("synap:  gesture_type=%d\n",gesture_type);
		#endif
			synaptics_rmi4_check_gesture(rmi4_data->input_dev,GESTURE_DOUBLECLICK);
		}
		else if ((rmi4_data->gesture_detection[2]==0x63)&&(rmi4_data->gesture_detection[0]==0x0b))
		{
		#if SYNAPTICS_PRINTK_DEBUG
			printk("synap:  gesture_type=%d\n",gesture_type);
		#endif
			synaptics_rmi4_check_gesture(rmi4_data->input_dev,GESTURE_C);
		}
		else if ((rmi4_data->gesture_detection[2]==0x65)&&(rmi4_data->gesture_detection[0]==0x0b))
		{
		#if SYNAPTICS_PRINTK_DEBUG
			printk("synap:  gesture_type=%d\n",gesture_type);
		#endif
			synaptics_rmi4_check_gesture(rmi4_data->input_dev,GESTURE_E);
		}
		else if ((rmi4_data->gesture_detection[2]==0x6d)&&(rmi4_data->gesture_detection[0]==0x0b))
		{
		#if SYNAPTICS_PRINTK_DEBUG
			printk("synap:  gesture_type=%d\n",gesture_type);
		#endif
			synaptics_rmi4_check_gesture(rmi4_data->input_dev,GESTURE_M);
		}
		else if ((rmi4_data->gesture_detection[2]==0x77)&&(rmi4_data->gesture_detection[0]==0x0b))
		{
		#if SYNAPTICS_PRINTK_DEBUG
			printk("synap:  gesture_type=%d\n",gesture_type);
		#endif
			synaptics_rmi4_check_gesture(rmi4_data->input_dev,GESTURE_W);
		}
		ret = easy_wakeup_gesture_report_position(rmi4_data);
		if (ret < 0)
		{
			printk("report position error!retval = %d \n",ret);
			return 0;
		}
...
void enable_irq_for_extern_usage(bool enable)
{
	synaptics_rmi4_irq_enable(synap_rmi4_data, enable, false);
}
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		...
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F1);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F8);
		...
		set_bit(KEY_F1, rmi4_data->input_dev->keybit);
		...
		set_bit(KEY_F11, rmi4_data->input_dev->keybit);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	mutex_lock(&synap_rmi4_data->sysfs_mutex);

	printk("%s: ft_easy_wakeup_gesture=0x%04x \n", __func__, synap_easy_wakeup_gesture);
	ret = snprintf(buf, PAGE_SIZE, "0x%04x\n",synap_easy_wakeup_gesture);

	mutex_unlock(&synap_rmi4_data->sysfs_mutex);
	return ret;
}
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret < 0){
		dev_err(synap_rmi4_data->pdev->dev.parent,
				"%s: kstrtoul error,ret=%d \n", __func__,ret);
		return ret;
	}
	...
	if (synap_easy_wakeup_gesture != ((u32)GESTURE_FROM_APP(value)))
		synap_easy_wakeup_gesture = (u32)GESTURE_FROM_APP(value);
		dev_err(synap_rmi4_data->pdev->dev.parent,
		"%s: ft_easy_wakeup_gesture=%d \n", __func__, synap_easy_wakeup_gesture);
...
static struct kobj_attribute easy_wakeup_gesture = {
	.attr = {.name = EASY_WAKEUP_GEASTURE_NAME, .mode = (S_IRUSR | S_IWUSR | S_IRGRP |S_IWGRP |S_IROTH | S_IWOTH) /*.mode = (S_IRUGO | S_IWUSR | S_IWGRP) */},
...
static struct kobject* tp_get_touch_screen_obj(struct synaptics_rmi4_data *rmi4_data)
{
	if( NULL == touch_screen_kobject_ts )
	{
		...
		{
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: create sys/touchscreen kobjetct error!\n", __func__);
			return NULL;
		}
		else
		{
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: create sys/touchscreen/ successful!\n", __func__);
		}
	}
	else
	{
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: sys/touchscreen/ already exist!\n", __func__);
	}
...
static int add_easy_wakeup_interfaces(struct synaptics_rmi4_data *rmi4_data)
{
	int error = 0;
	struct kobject *properties_kobj;

	properties_kobj = tp_get_touch_screen_obj(rmi4_data);
	if( NULL == properties_kobj )
	{
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: Error, get kobj failed!\n", __func__);
		return -1;
	}
	...
	if (error)
	{
		...
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: easy_wakeup_gesture create file error\n", __func__);
		return -ENODEV;
	}
	...
	{
		kobject_put(properties_kobj);
		dev_err(rmi4_data->pdev->dev.parent,"%s: easy_wakeup_position create file error\n", __func__);
		return -ENODEV;
	}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	const struct synaptics_dsx_board_data *bdata;
	if(strstr(panelid, "td4100") == NULL)
	{
		dev_err(&pdev->dev, "panel id not matched\n");
		return -1;
	}
...
#ifdef SYNAPTICS_GESTURE_SUPPROT
	mutex_init(&rmi4_data->sysfs_mutex);
	add_easy_wakeup_interfaces(rmi4_data);
#endif
...
bool gestrue_irq_wake_enble = false;
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		...
		if(synap_gesture_onoff() && gestrue_irq_wake_enble == false)
		{
			gestrue_irq_wake_enble = true;
			synaptics_rmi4_wakeup_gesture(rmi4_data, true);
			enable_irq_wake(rmi4_data->irq);
			goto exit;
		}
		else if(gestrue_irq_wake_enble == true)
		{
			gestrue_irq_wake_enble = false;
			synaptics_rmi4_wakeup_gesture(rmi4_data, false);
			disable_irq_wake(rmi4_data->irq);
		}
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		...
		if(synap_gesture_onoff() && gestrue_irq_wake_enble == true)
		{
			gestrue_irq_wake_enble = false;
			synaptics_rmi4_wakeup_gesture(rmi4_data, false);
			disable_irq_wake(rmi4_data->irq);

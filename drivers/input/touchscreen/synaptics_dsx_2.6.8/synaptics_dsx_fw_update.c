...
//Add,@wangdongbo.wt_20160628
#define WT_ADD_CTP_INFO	0

#define HW_ADD_CTP_INFO	1

#if  WT_ADD_CTP_INFO
//#include <linux/hardware_info.h>
static char tp_string_version[40];
static unsigned int Device_Fw_Id;
static unsigned char fw_str[32]={0};
static unsigned int Image_Fw_Id;
static unsigned char cfg_str[32]={0};
static void fwu_cfg_info_show(void);
#endif
...
static unsigned int Image_Fw_Id;
static unsigned char cfg_str[32]={0};
static void ctp_app_info(void);
#endif

//#define FW_IHEX_NAME "synaptics_fw.bin"
//#define FW_IMAGE_NAME "synaptics_fw.img"
#define FW_IHEX_B3_NAME 	"synaptics_fw_b3.bin"
#define FW_IHEX_B8_NAME 	"synaptics_fw_b8.bin"
#define FW_IMAGE_B3_NAME 	"synaptics_fw_b3.img"
#define FW_IMAGE_B8_NAME 	"synaptics_fw_b8.img"
extern int get_mdss_dsi_panel_name(void);
...
static void fwu_parse_image_header_05_06(void)
{
	...
	//Modify for compatible with optimize & non-optimize fw
...
static int fwu_get_device_config_id(void)
{
	...
	for(i=0;i<config_id_size;i++)
		dev_info(rmi4_data->pdev->dev.parent,"synaptics:  fwu->config_id[%d] = 0x%x \n",i,fwu->config_id[i]);

	if (retval < 0)
		return retval;

	//Modify,@wangdongbo.wt_for recovery tp fw
	if( (config_id_size == V5V6_CONFIG_ID_SIZE)
	   &&(fwu->config_id[0] == 0xFF) )
	{
		fwu->config_id[0] = 0;
		fwu->config_id[1] = 0;
		fwu->config_id[2] = 0;
		fwu->config_id[3] = 0;
		dev_info(rmi4_data->pdev->dev.parent,"synaptics:  fwu->config_id[0] = 0x%02x \n",fwu->config_id[0]);
		dev_info(rmi4_data->pdev->dev.parent,"synaptics:  fwu->config_id[1] = 0x%02x \n",fwu->config_id[1]);
		dev_info(rmi4_data->pdev->dev.parent,"synaptics:  fwu->config_id[2] = 0x%02x \n",fwu->config_id[2]);
		dev_info(rmi4_data->pdev->dev.parent,"synaptics:  fwu->config_id[3] = 0x%02x \n",fwu->config_id[3]);
	}
	return 0;
}
...
/*
snprintf(fw_str, HARDWARE_MAX_ITEM_LONGTH, "%08X", Device_Fw_Id);
sprintf(tp_string_version, "Synaptics,FW:%s", fw_str);
...
*/
	...
	for (ii = 0; ii < config_id_size; ii++) {
		dev_err(rmi4_data->pdev->dev.parent,"synaptics:  fwu->img.ui_config.data[%d] =0x%x \n",ii,fwu->img.ui_config.data[ii] );
...
{
	snprintf(fw_str, HARDWARE_MAX_ITEM_LONGTH, "%08X", Device_Fw_Id);
	snprintf(cfg_str, HARDWARE_MAX_ITEM_LONGTH, "%02X%02X%02X%02X", fwu->config_id[0],fwu->config_id[1],fwu->config_id[2],fwu->config_id[3]);
	sprintf(tp_string_version, "Synaptics,FW:%s,cfg:%s", fw_str,cfg_str);
	hardwareinfo_set_prop(HARDWARE_TP, tp_string_version);
}
#endif
...
{
	snprintf(fw_str, HARDWARE_MAX_ITEM_LONGTH, "%08X", Device_Fw_Id);
	snprintf(cfg_str, HARDWARE_MAX_ITEM_LONGTH, "%X%X%X%X", fwu->config_id[0],fwu->config_id[1],fwu->config_id[2],fwu->config_id[3]);
	sprintf(tp_string_version, "Synaptics,FW:%s,cfg:%s", fw_str,cfg_str);
	app_info_set("touch_panel", tp_string_version);
}
#endif
...
static int fwu_start_reflash(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
#ifdef CONFIG_HUAWEI_DSM
	struct dsm_client *touch_dclient = tp_dsm_get_client();
#endif
	char image_name[MAX_IMAGE_NAME_LEN]={0};
	...
	if (fwu->image == NULL) {
		//Modify,@wangdongbo,wt_for_compatible_with_BOE_B3_B8
		retval = get_mdss_dsi_panel_name();
		if(retval == 0)
		{
			dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to get panel name\n",__func__);
			goto exit;
		}
		dev_err(rmi4_data->pdev->dev.parent,"wdb: retval=%d \n",retval);
		if(retval == 1)//B3_1
		{
			strcpy(image_name, FW_IMAGE_B3_NAME);
		}
		else if(retval == 2)//B3_2
		{
			strcpy(image_name, FW_IMAGE_B3_NAME);
		}
		else if(retval == 3)//B8+
		{
			strcpy(image_name, FW_IMAGE_B8_NAME);
		}
		else
		{
			dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to get panel name\n",__func__);
		...
		dev_err(rmi4_data->pdev->dev.parent,"wdb: image_name=%s \n",image_name);
	...
	//Add,@wangdongbo.wt_skip_upgrade_fw in factory mode
	if(get_boot_mode(rmi4_data) != 0)
		flash_area = NONE;
...
#ifdef CONFIG_HUAWEI_DSM
	if(retval<0) {
		dsm_client_record(touch_dclient,"touch firmware upgrade fail retval = %d\n", retval);
...
static int fwu_start_recovery(void)
{
	...
	if (fwu->image == NULL) {
		...
		dev_err(rmi4_data->pdev->dev.parent," retval=%d \n",retval);
		if(retval == 1)//B3_1
		{
			strcpy(image_name, FW_IHEX_B3_NAME);
		}
		else if(retval == 2)//B3_2
		{
			strcpy(image_name, FW_IHEX_B3_NAME);
		...
		{
			strcpy(image_name, FW_IHEX_B8_NAME);
		}
		else
		{
			dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to get panel name\n",__func__);
		...
		dev_err(rmi4_data->pdev->dev.parent," image_name=%s \n",image_name);
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				image_name, strlen(image_name),
				strlen(image_name));
		if (retval < 0) {
...
//Modify, wangdongbo.wt_20160626_for_factory_skip_upgrade
#ifdef DO_STARTUP_FW_UPDATE
static int get_boot_mode(struct synaptics_rmi4_data *rmi4_data)
{
        int ret;
...
*/
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
        ...
        dev_err(rmi4_data->pdev->dev.parent,"synaptics start upgrade thread \n");

...
#define SYN_I2C_NUM		   	 	3
#define SYN_I2C_RETRY_TIMES  		5*SYN_I2C_NUM
#define SYNAPTICS_BOE_B3_B8_COMPABITLE		1
...
#ifdef CONFIG_HUAWEI_DSM
static struct dsm_dev dsm_touch_synaptics = {
	.name = "dsm_i2c_bus",
//	.device_name = "synaptics",
	.ic_name = "NNN",
	.module_name = "NNN",
	.fops = NULL,
	.buff_size = 1024,
...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	bdata->irq_gpio = of_get_named_gpio_flags(np,"synaptics,irq-gpio", 0, (enum of_gpio_flags *)&bdata->irq_flags);
		...
		struct i2c_client *i2c)
{
	...
	if (hw_if.board_data->i2c_addr == 0x4b)
		hw_if.board_data->i2c_addr = 0x70;
	else if (hw_if.board_data->i2c_addr == 0x70)
		...
		unsigned short addr)
{
	...
	if (page != rmi4_data->current_page) {
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retry == i2c_num*(SYN_I2C_RETRY_TIMES / SYN_I2C_NUM)) {
...
#ifdef CONFIG_HUAWEI_DSM
	if(retval<0) {
		dsm_client_record(touch_dclient,"i2c write err retval = %d\n", retval);
		dsm_client_notify(touch_dclient, DSM_TP_I2C_RW_ERROR_NO);
	}
#endif/*CONFIG_HUAWEI_DSM*/
	return retval;
...
#if SYNAPTICS_BOE_B3_B8_COMPABITLE
extern int get_mdss_dsi_panel_name(void);
int check_mdss_dsi_panel_type(struct i2c_client *client)
{
	int retval;
	retval = get_mdss_dsi_panel_name();
	if (retval == 3)//B8
	{
		dev_err(&client->dev,"synaptics: Lcm panel type : B8 \n");
		return 1;
	}
	dev_err(&client->dev,"synaptics: Lcm panel type : B3 \n");
	return 0;
}
#endif
		...
		const struct i2c_device_id *dev_id)
{
    ...
    if(!touch_dclient)
    {
        touch_dclient = dsm_register_client(&dsm_touch_synaptics);
    }
#endif
...
#if  SYNAPTICS_BOE_B3_B8_COMPABITLE
retval = check_mdss_dsi_panel_type(client);
if (retval == 1)//B8
{
	client->addr = 0x70;//check panel b8, modify i2c addr
}
#endif
...
#ifdef  CONFIG_HUAWEI_DSM
	dsm_unregister_client(touch_dclient,&dsm_touch_synaptics);
#endif
	return 0;
}
